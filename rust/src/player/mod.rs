...
pub fn spawn(initial_w: u32, initial_h: u32) -> anyhow::Result<PlayerHandle> {
    ...
    thread::Builder::new().name("mpv-player".into()).spawn(move || {
        let mpv = match Mpv::new() {
            Ok(m) => m,
            Err(e) => {
                let _ = evt_tx.send(Event::Error { msg: format!("mpv init: {}", e) });
                return;
            }
        };
        let mut rctx = match RenderCtx::new(&mpv, frames_for_thread, initial_w.max(1), initial_h.max(1)) {
            Ok(r) => r,
            Err(e) => {
                let _ = evt_tx.send(Event::Error { msg: format!("render ctx: {}", e) });
                return;
            }
        };
        ...
        loop {
            ...
            if rctx.has_new_frame() {
                if let Err(e) = rctx.render_into_frames() {
                    ...
                    let _ = evt_tx.send(Event::Error { msg: format!("render: {}", e) });
